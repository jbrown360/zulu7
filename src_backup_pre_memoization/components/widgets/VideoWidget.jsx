    ...
    if (data.type === 'iframe' || data.type === 'camera' || data.type === 'proxy' || data.type === 'web' || data.type === 'integration') {
        ...
        return (
            <div ref={containerRef} className={`flex flex-col relative group overflow-hidden bg-black ${isFullscreen ? 'fixed inset-0 z-[9999] w-screen h-screen' : 'h-full w-full'}`}>
                ...
                {isLocked && (
                    <div className="absolute top-1.5 right-3.5 z-50 flex items-center bg-[#0a0f18]/90 rounded-none border border-white/20 no-drag overflow-hidden">
                        {data.type === 'iframe' && (
                            <>
                ...
                <div className="flex-1 relative w-full h-full overflow-hidden bg-[#0d0d12]">
                    ...
                    {data.type === 'camera' && (
                        <div className="absolute bottom-2 left-2 flex items-center space-x-1.5 z-10 px-2 py-1 bg-black/80 rounded-none border border-white/5 pointer-events-none">
